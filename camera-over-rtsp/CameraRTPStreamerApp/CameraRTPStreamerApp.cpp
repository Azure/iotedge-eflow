...
uint16_t SecureServerPort = 6554;
...
// Uncomment the following if you want to use the user creation tool
//#define USE_USER_CREATOR
...
int main(int argc, char* argv[])
{
	...
	std::cout << "********************************\n";
	std::cout << "** If using arguments, first argument should be the Device Index number and second argument should be the Resolution Index number **\n";

	if (argc == 4) {
		argsPassed = true;
		std::cout << "Camera device entered: " << argv[1] << "\n";
		std::cout << "Camera media format entered: " << argv[2] << "\n";
		std::cout << "Port entered: " << argv[3] << "\n";
	}
	...
	try
	{
		...
		if (argsPassed) {
			...
			int deviceSelection = atoi(argv[1]);
		
			if (deviceSelection <= filteredDevices.Size()) {
				DeviceInformation device = filteredDevices.GetAt(deviceSelection);
				std::wcout << std::endl << "Device Selected: " << device.Name().c_str();
				s.VideoDeviceId(device.Id());
			}
			else{
				throw_hresult(MF_E_NO_CAPTURE_DEVICES_AVAILABLE);
			}
		...
		if (argsPassed) {
			...
			if (!formats.Size())
			{
				throw_hresult(MF_E_NO_MORE_QUALITY_LEVELS);
			}

			int formatSelection = atoi(argv[2]);

			if (formatSelection <= formats.Size()) {
				selectedProp = formats.GetAt(formatSelection);
				auto format = selectedProp.try_as<VideoEncodingProperties>();
				float fr = (float)format.FrameRate().Numerator() / (float)format.FrameRate().Denominator();
				std::wcout << L"\n" << "Format Selected: "<< format.Width() << L"x" << format.Height() << L"@" << fr << L":" << format.Subtype().c_str();
				mc.VideoDeviceController().SetMediaStreamPropertiesAsync(MediaStreamType::VideoRecord, selectedProp).get();
			}
			else {
				throw_hresult(MF_E_NO_CAPTURE_DEVICES_AVAILABLE);
